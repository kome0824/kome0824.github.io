document.getElementById("version-short").innerHTML = "[BW ver 0.3.0.0]";
...
function afterDxLibInitialize() { // DxLib初期化に合わせて一回鳴らしておく
    playCheckSound();
}
...
// 設定ボタン
{
    ...
    function limitedX(xPos, el){
        ...
        return Math.max(window.innerWidth * 0.1, Math.min(xPos, window.innerWidth - rect.width  - window.innerWidth * 0.0125));
    }

    function limitedY(yPos, el){
        const rect = el.getBoundingClientRect();
        return Math.max(0, Math.min(yPos, window.innerHeight - rect.height - window.innerHeight * 0.1));
    }
